...
export function serializeClearLinkAccessCookie(slug) {
  const secure = process.env.NODE_ENV === 'production' ? '; Secure' : ''
  return `${getLinkCookieName(
    slug
...
export function serializeClearLinkAccessCookies(slugs) {
  const unique = [...new Set((Array.isArray(slugs) ? slugs : [slugs]).filter(Boolean))]
  return unique.map((slug) => serializeClearLinkAccessCookie(slug))
}
